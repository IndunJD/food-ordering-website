...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if(selectTab === 'foods') {
      const filteredFoods = foodMenu.filter(item => item.category === 'foods')
      setMenu(filteredFoods)
    }

    if(selectTab === 'beverages') {
      const filteredFoods = foodMenu.filter(item => item.category === 'beverages')
      setMenu(filteredFoods)
    }
  },[selectTab])
      ...
      <div className="w-full h-full bg-blackish px-[100px] m-auto pt-[50px]">
        <div className="flex items-center justify-between flex-wrap">
          ...
          <div className="flex gap-3">
            <button onClick={() => setSelectTab('all')} className="text-whiteish border border-solid border-secondary py-2 px-4 rounded-md feedback-card font-raleway">
              All
            </button>
            <button onClick={() => setSelectTab('foods')} className="text-whiteish border border-solid border-secondary py-2 px-4 rounded-md feedback-card font-raleway">
              Foods
            </button>
            <button onClick={() => setSelectTab('beverages')} className="text-whiteish border border-solid border-secondary py-2 px-4 rounded-md feedback-card font-raleway">
              Beverages
            </button>
          </div>
        </div>
            ...
            >
              ...
              <div className="w-full h-full bg-darkish bg-opacity-70 absolute top-0 left-0 z-[5] hidden group-hover:block ">
                <div className="w-full h-full flex items-center justify-center">
                  <button onClick={() => showModalHandler(menu.id)} className="font-raleway uppercase sm:flex hidden bg-gradient-to-r from-primary to-secondary text-whiteish text-sm font-semibold py-2 px-6 rounded-lg cursor-pointer">
                    View
                  </button>
                </div>
              </div>
